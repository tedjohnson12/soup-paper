...
\begin{abstract}
    ...
    \begin{minipage}{0.25\textwidth}
        \includegraphics[width=\textwidth]{soup.pdf}
    \end{minipage}
\end{abstract}
...
\section{Budget}
The SOUP budget is summarized in Table \ref{tab:budget}.
...
\begin{table}
    ...
    \begin{tabular}{lc}
        ...
        {\bf Total} & 325\\
        \hline \hline
        Extended Mission & $2.5\;\mathrm{yr^{-1}}$
    \end{tabular}
    \caption{SOUP Budget Summary. Costs given in millions of \$US.}
    \label{tab:budget}
\end{table}

\subsection{Budget Justification}

This budget justification is largely based on peer missions Kepler and TESS, and an analysis of space telescope costs by H. Philip Stahl at NASA MSFC\footnote{\url{https://ntrs.nasa.gov/api/citations/20110015780/downloads/20110015780.pdf}}.

\subsubsection{Instruments}
A primary advantage of the SOUP strategy is to use technology that is already mature. For space telescopes, the cost of a mission is proportional to the technology readiness level (TRL) to the power of 0.9. SOUP relies on detector technology matured by Kepler and optical and near-UV dichroics that can be manufactured at low cost. Therefore, our \$10M instrument cost is reasonable.

\subsubsection{Spacecraft}
The SOUP spacecraft must be large to host both the \diam{} mirror and the three sets of optical tracks and detector arrays. Additionally, the Earth-trailing heliocentric orbit will require a large high-gain antenna in order to communicate with the Deep Space Network (DSN). Spacecraft assembly is generally 25\% of mission cost. We estimate a cost of \$80M for the SOUP spacecraft.

\subsubsection{Launch Vehicle}
With a \diam{} mirror and large spacecraft requirement, SOUP is significanlty more massive than Kepler, but not as massive as larger-aperture multi-instrument observatories like the Hubble Space Telescope. However, the requirement to escape the Earth's gravitational well requires more fuel per kg than HST's low-Earth orbit. Therefore, we adopt the geometric mean of the two observatories' launch costs of \$100M.

\subsubsection{Science Team}
The science budget's primary goal is to fund the team that analyzes SOUP's data and validates any discoveries the observatory makes. Due to the high data volume expected, this team's budget will be on the order of \$2M yr$^{-1}$. Over the course of the primary mission, this cost is \$20M.

\subsubsection{Mission Operations}
The SOUP spacecraft will not be serviceable, but the length of the primary mission will still require a relatively high operations budget. We predict a cost of \$40M over the \lifetime year primary mission.  

\subsubsection{Extended Mission}
We propose that the SOUP mission could be extended at a reduced cost of $\$ 2.5 \mathrm{M~yr^{-1}}$ with the aim of discovering long-period exoplanets that require a long baseline. The length of this primary mission will be determined by the observatories continued ability to downlink data to Earth as it drifts further away in its orbit.

\section{Timeline}
The SOUP timeline is as follows:

% \begin{center}
%     \begin{tabular}{l}
\vspace{2mm}
\noindent
        {\em 2026} \dotfill Propose Mid-sized Explorer-class mission\\
        {\em 2027 Q1--2} \dotfill Optics bench testing and development \\
        {\em 2027 Q1--2} \dotfill Primary mirror design and development \\
        {\em 2027 Q3} \dotfill Final instrument design \\
        {\em 2028 Q1--2} \dotfill Spacecraft design \\
        {\em 2028 Q3-2029 Q1} \dotfill Spacecraft assembly \\
        {\em 2029 Q3} \dotfill Launch \\
        {\em 2029 Q4--2030 Q1} \dotfill Commissioning \\
        {\em 2030--2040} \dotfill Primary mission \\
        {\em 2040--\phantom{2050}} \dotfill Extended mission\\
%     \end{tabular}
% \end{center}
